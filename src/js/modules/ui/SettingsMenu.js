export class SettingsMenu {
  constructor() {
    ...
    // Cached elements
    this.modal = null;
    this.btnSettingsToggle = null;      // From main menu
    this.btnGameSettingsToggle = null;  // From in-game menu/hangar selector
    this.btnSave = null;
    this.btnClose = null;
  ...
  init(engine) {
    ...
    // Cache DOM elements from index.html
  ...
  bindEvents() {
    ...
    if (this.btnGameSettingsToggle) {
      this.btnGameSettingsToggle.addEventListener('click', () => {
        // Retrieve Menu module and close it cleanly to reset its active state
    ...
    // Dynamic slider label updates during manual configuration adjustments
  ...
  closeSettings() {
    ...
    this.loadSavedConfiguration(); // Rollback unsaved slider changes to actual active states
  }

  loadSavedConfiguration() {
    // Read local storage falling back to defaults [3]
  ...
  saveConfiguration() {
    // Persist values in browser local storage [3]
  ...
  applyVolumesRealtime() {
    ...
    if (soundManager) {
      ...
      // If muted, drop master to 0.0, else restore slider setting
    ...
    // Update global exported simulation parameters
