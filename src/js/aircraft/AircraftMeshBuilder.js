...
export class AircraftMeshBuilder {
  static build(aircraft) {
    ...
    if (aircraft.config.modelPath) {
      // Load 3D model asset
      this.buildFromModel(aircraft);
  ...
  static buildFromModel(aircraft) {
    // 1. Create a wireframe bounding box matching dimensions as an elegant neon visual loading placeholder
    const dim = aircraft.config.dimensions || { span: 10, length: 10, height: 3 };
    const placeholderGeo = new THREE.BoxGeometry(dim.span, dim.height, dim.length);
    ...
    aircraft.group.add(placeholderMesh);

    // 2. Load the asynchronous glTF/glb file
    const loader = new GLTFLoader();
    loader.load(
      aircraft.config.modelPath,
      (gltf) => {
        // Remove the visual loader placeholder on load completion
        aircraft.group.remove(placeholderMesh);
        placeholderGeo.dispose();
        placeholderMat.dispose();

        const modelScene = gltf.scene;
        this.configureShadows(modelScene);

        // 3. Scan the glTF scene hierarchy to dynamically assign animation structures
        modelScene.traverse((child) => {
          const nodeName = child.name.toLowerCase();

          // Search for propeller geometries (e.g., 'propeller', 'prop_blade')
          if (nodeName.includes('propeller') || nodeName.includes('prop')) {
            aircraft.propellerGroup = child;
          }

          // Search for dynamic gears (reparks them to the central gear group toggle)
          if (nodeName.includes('gear') || nodeName.includes('wheel')) {
            aircraft.gearGroup.add(child);
          }

          // Search for afterburner or jet flame effects
          if (nodeName.includes('afterburner') || nodeName.includes('flame') || nodeName.includes('exhaust')) {
            aircraft.afterburnerGroup = child;
            aircraft.afterburnerGroup.visible = false;
          }
        });

        aircraft.group.add(modelScene);
        console.log(`[AircraftMeshBuilder] Loaded 3D model: ${aircraft.config.name}`);
      },
      (xhr) => {
        // Optional download progress logging
        const percent = Math.round((xhr.loaded / xhr.total) * 100);
        if (percent % 25 === 0 && isFinite(percent)) {
          console.log(`[AircraftMeshBuilder] Fetching model "${aircraft.config.name}": ${percent}%`);
        }
      },
      (error) => {
        // Graceful failure fallback: keeps the frame active and marks the bounding box red
        console.error(`[AircraftMeshBuilder] Failed loading GLTF model from ${aircraft.config.modelPath}:`, error);
        placeholderMesh.material.color.setHex(0xff3333);
      }
    );
  }
